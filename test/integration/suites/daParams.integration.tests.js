...
export function registerDaParamsIntegrationTests({
  ...
  describe('DA params', () => {
    const fdaId = 'fda_params_fixture';
    const daId2 = 'da_params_main';
    ...
    beforeAll(async () => {
      ...
      const createFda = await httpReq({
        ...
        body: {
          ...
          description: 'da params fixture',
        },
      });

      if (createFda.status !== 202) {
        throw new Error(
          `Failed to create params fixture FDA: ${createFda.status} ${JSON.stringify(createFda.json)}`,
        );
      }
