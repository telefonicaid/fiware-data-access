...
| 500  | Internal Server Error  | `DefaultDataAccessCreationError` | Creation of the automatic `defaultDataAccess` failed during `POST /fdas`. FDA provisioning is rolled back atomically (bootstrap objects and the FDA metadata document are removed). See [Default Data Access](AdvancedTopics/default_data_access.md#atomic-behavior).                                              |
...
| `schema`    | ✓        | array  | `[{name, type}]` describing the FDA columns, e.g. `[{"name":"age","type":"INTEGER"}]`. For cached FDAs created in `strict` mode. PostgreSQL FDAs use the actual database types; MongoDB FDAs use `null` as the type until the first fetch, when the schema is derived from the materialized Parquet. `type` value refers to DuckDB (the underlying DB technology) types. |
...
| `default`  | ✓        | string  | Provide a default value for the param in case it isn't provided. It is applied only when the value is missing and `required` is `false`. If omitted, no default is applied. It can be set to `null` for any `type`, in which case the value passed to the query when the param is missing is `NULL`, useful for optional filters such as `WHERE ($param IS NULL OR field = $param)`. |
