...
| `fetchSize`       |          | string | **Only for type `window`**, it can take the values `hour`, `day`, `week` and `month`. Represents the time range of data to fetch (e.g. last hour/month data). Must be equal to partition size. |
