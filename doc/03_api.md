...
| `schema`    | ✓        | array  | `[{name, type}]` describing the FDA columns, e.g. `[{"name":"age","type":"INTEGER"}]` for PostgreSQL or `[{"name":"age","type":"VARCHAR"}]` for MongoDB. For cached FDAs created in `strict` mode. PostgreSQL FDAs use the actual database types; MongoDB FDAs use `VARCHAR` as a placeholder type. |
