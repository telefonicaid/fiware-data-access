...
[
  {
    ...
    "das": {
      "defaultDataAccess": {
        ...
        "query": "SELECT *, COUNT(*) OVER() as __total WHERE ($entityid IS NULL OR \"entityid\" IN (SELECT unnest(string_split($entityid, ',')))) AND ($entitytype IS NULL OR \"entitytype\" IN (SELECT unnest(string_split($entitytype, ',')))) AND ($alertdescription IS NULL OR \"__ALERTDESCRIPTION__\" IN (SELECT unnest(string_split($alertdescription, ',')))) AND ($name IS NULL OR \"__NAME__\" IN (SELECT unnest(string_split($name, ',')))) AND ($severity IS NULL OR \"__SEVERITY__\" IN (SELECT unnest(string_split($severity, ',')))) AND ($time_between_notif IS NULL OR \"__TIME_BETWEEN_NOTIF__\" IN (SELECT unnest(string_split($time_between_notif, ',')))) AND ($templateid IS NULL OR \"templateid\" IN (SELECT unnest(string_split($templateid, ',')))) AND ($attr IS NULL OR \"__ATTR__\" IN (SELECT unnest(string_split($attr, ',')))) AND ($oper IS NULL OR \"__OPER__\" IN (SELECT unnest(string_split($oper, ',')))) AND ($umbral IS NULL OR \"__UMBRAL__\" IN (SELECT unnest(string_split($umbral, ',')))) AND ($created_at IS NULL OR \"created_at\" IN (SELECT unnest(string_split($created_at, ',')))) LIMIT CAST($pageSize AS BIGINT) OFFSET CAST($pageStart AS BIGINT)",
        "params": [
          {
            "name": "entityid",
            "default": null
