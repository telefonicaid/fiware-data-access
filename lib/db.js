...
import { getProcessedFilePath, isCSV, processHeader } from './csv.js';
import { getSet } from './mongo.js';
...
export async function createTable(conn, tableName, filePath) {
  console.log(`Creating table '${tableName}' in DuckDB...`);

  if (isCSV(filePath)) {
    processHeader(filePath);
    filePath = getProcessedFilePath(filePath);
  }
...
export async function runPreparedStatement(conn, path, params) {
  if (!getPreparedStatement(path)) {
    const { queryTemplate } = await getSet(path);
    if (!queryTemplate) throw `FDA ${path} does not exist in Mongo.`;
    await storePreparedStatement(conn, path, queryTemplate);
  }

  const dbStatement = getPreparedStatement(path);
...
export function getPreparedStatement(path) {
  return preparedStatements.get(path);
}

export async function storePreparedStatement(conn, fda, queryTemplate) {
  const dbStatement = await conn.prepare(queryTemplate);
  preparedStatements.set(fda, dbStatement);
}
