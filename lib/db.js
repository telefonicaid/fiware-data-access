...
export async function runPreparedStatement(conn, service, setId, id, params) {
  if (!getPreparedStatement(service, setId, id)) {
    const fda = await getFDA(service, setId, id);
    if (!fda?.query)
      throw `FDA ${id} does not exist in set ${setId} with service ${service}.`;
    await storePreparedStatement(conn, service, setId, id, fda.query);
  }

  const dbStatement = getPreparedStatement(service, setId, id);
...
export function getPreparedStatement(service, setId, id) {
  return preparedStatements.get(`${service}${setId}`)?.get(id);
}
