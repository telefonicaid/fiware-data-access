...
export async function runPreparedStatement(conn, service, fdaId, daId, params) {
  if (!getPreparedStatement(service, fdaId, daId)) {
    ...
    if (!da?.query) {
      const errMessage = `DA ${daId} does not exist in FDA ${fdaId} with service ${service}.`;
  ...
  try {
    ...
    const errMessage = `Error running the prepared statement: ${e}`;
    console.log(errMessage);
    setResponse(500, 'DuckDBServerError', errMessage);
    throw e;
  }
}
...
export async function storePreparedStatement(
  ...
  try {
    ...
    const errMessage = `Error storing the prepared statement in the fda ${fdaId} with service ${service}: ${e}`;
    console.log(errMessage);
    setResponse(500, 'DuckDBServerError', errMessage);
    throw e;
  }
