...
export async function createIndex() {
  ...
  collection.createIndex({ setId: 1, service: 1 }, { unique: true });
}
...
export async function createSet(setId, table, bucket, path, service) {
  ...
  try {
    ...
    if (e.code === 11000) {
      console.log(`Set with id ${setId} and ${service} already exists.`);
    } else {
...
export async function storeFDA(service, setId, id, description, query) {
  const collection = await getCollection();
  try {
    ...
    console.log(
      `Error storing FDA ${id} in set ${setId} of service ${service}: ${e}`
    );
  }
}
...
export async function getFDA(service, setId, id) {
  const collection = await getCollection();
