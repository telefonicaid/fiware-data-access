...
export async function storeSetPG(bucket, database, table, fda) {
  // cambiar el orden
  await performance(bucket, database, table, fda);
  console.log('#=======Starting Complete execution=======#');
  console.time('completeExecution');

  await queryPG(bucket, database, table, fda);
  const conn = await getDBConnection('localhost:9000', 'admin', 'admin123');
  await toParquet(
    conn,
  ...
  console.timeEnd('completeExecution');
  console.log('=======Ended completeExecution=======');
}
