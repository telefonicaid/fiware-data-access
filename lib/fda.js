...
export async function queryFDA({ path, ...params }) {
...
export async function storeSetPG(database, table, bucket, fda, query) {
  ...
  const conn = await getDBConnection('localhost:9000', 'admin', 'admin123');
  const parquetPath = getPath(bucket, fda, 'parquet');
