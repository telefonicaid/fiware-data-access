...
export async function fetchFDA(
  ...
  await uploadTableToMinio(database, schema, table, bucket, path);
  await createFDA(fdaId, database, schema, table, bucket, path, service);
}

export async function updateFDA(service, fdaId) {
  const { database, schema, table, bucket, path } = await retrieveFDA(
    service,
    fdaId
  );
  await uploadTableToMinio(database, schema, table, bucket, path);
...
export async function deleteFDA(service, fdaId) {
  const { bucket, path } = (await retrieveFDA(service, fdaId)) ?? {};
  if (!bucket || !path) {
    return 404;
  }
  ...
  await dropFile(s3Client, bucket, path);
  return await removeFDA(service, fdaId);
}

export async function getDAs(service, fdaId) {
...
async function uploadTableToMinio(database, schema, table, bucket, path) {
  const s3Client = await getS3Client(
    'http://localhost:9000',
    'admin',
    'admin123'
  );
  await uploadTable(s3Client, bucket, database, schema, table, path);
