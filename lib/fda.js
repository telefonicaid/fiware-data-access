...
import { createFDA, getFDAs, storeDA } from './mongo.js';

export async function listFDAs(service) {
  return getFDAs(service);
}

export async function queryFDA(service, { fdaId, daId, ...params }) {
...
export async function fetchFDA(fdaId, database, table, bucket, path, service) {
  ...
  await createFDA(fdaId, table, bucket, path, service);
}
