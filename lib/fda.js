...
import { createSet, getFDAs, storeFDA } from './mongo.js';

export async function listFDAs(service) {
  return getFDAs(service);
}

export async function querySet(service, { setId, id, ...params }) {
  const conn = await getDBConnection('localhost:9000', 'admin', 'admin123');

  const queryRes = await runPreparedStatement(conn, service, setId, id, params);
  return queryRes;
}
