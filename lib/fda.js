...
export async function fetchFDA(
  ...
  await uploadTableToObjStg(database, query, service, path);
  await createFDA(fdaId, database, query, path, service, description);
}

export async function updateFDA(service, fdaId) {
  const { database, query, path } = await retrieveFDA(service, fdaId);
  await uploadTableToObjStg(database, query, service, path);
}
...
export async function getDA(service, fdaId, daId) {
  ...
  if (da) {
    ...
    const errMessage = `DA ${daId} not found in FDA ${fdaId} and service ${service}.`;
    console.log(errMessage);
    setResponse(404, 'DaNotFound', errMessage);
    throw new Error(errMessage);
  }
